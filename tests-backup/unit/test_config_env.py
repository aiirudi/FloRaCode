...
def test_dotenv_base_loaded(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _write_env(env_file, "FLORA_PORT=9999\n")
...
def test_system_env_overrides_dotenv(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _write_env(env_file, "FLORA_PORT=9999\n")
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("FLORA_PORT", "8888")
...
# .env 中设置的 FLORA_CONFIG 能影响 TOML 文件加载路径
def test_dotenv_before_toml_flora_config(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    toml_path = tmp_path / "custom.toml"
    toml_path.write_bytes(b'[core]\nport = 5555\n')

    env_file = tmp_path / ".env"
    _write_env(env_file, f"FLORA_CONFIG={toml_path}\n")
    monkeypatch.chdir(tmp_path)
    monkeypatch.delenv("FLORA_CONFIG", raising=False)
    monkeypatch.delenv("FLORA_PORT", raising=False)
...
def test_priority_chain_full(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    env_file = tmp_path / ".env"
    _write_env(env_file, "FLORA_PORT=7000\n")
