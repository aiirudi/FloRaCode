...
from flora_claude.core.llm.types import LlmResponse, UsageStats
...
async def test_emits_api_call_and_api_response(tmp_path: Path) -> None:
    ...
    provider = TraceProvider(inner, writer)
    bus = EventBus()
...
async def test_include_payload_true_embeds_full_content(tmp_path: Path) -> None:
    ...
    provider = TraceProvider(inner, writer, include_payload=True)
    await provider.chat(
...
async def test_include_payload_false_uses_summary(tmp_path: Path) -> None:
    ...
    provider = TraceProvider(inner, writer, include_payload=False)
    await provider.chat(
...
# 功能：验证 TraceProvider 将 step 参数透传给 inner provider
