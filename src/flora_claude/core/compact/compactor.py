...
from flora_claude.core.bus.events import ContextCompactedEvent
...
class Compactor:
    ...
    async def compact(
        ...
        result = await self.compact_messages(
            context.messages, provider, focus
        )
        if result is None:
            return None
        ...
        logger.info("context compact session=%s run_id=%s original≈%d summary=%d", 
                    self._session_id, context.run_id, result.original_token_estimate, result.summary_tokens)
        return result


    # 纯函数式压缩：接收消息列表，返回 CompactionResult; 失败时返回消息 None
    async def compact_messages(
        ...
        from flora_claude.core.events.bus import EventBus as _Bus
        ...
        compact_request: list[dict[str, object]] = [
    ...
    # 将状态摘要(State Summary) 写入 session 目录的 session_<ts>.md
    def _write_summary(self, text: str) -> None:
        try:
            ...
            logger.exception("compactor: ailed to write summary file")
