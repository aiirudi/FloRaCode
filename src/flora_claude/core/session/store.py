...
class SessionStore:
    ...
    # 追加一条 Anthropic API 消息到 thread.jsonl 中
    def append_message(
        self,
        sid: str,
        role: str,
        ...
        with (path / "thread.jsonl").open(mode="a",encoding="utf-8") as f:
    ...
    def read_messages(self, sid: str) -> list[dict[str, Any]]:
        ...
        for line_no, line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
            ...
            if role not in ("user", "assistant"):
                logger.warning("skip unknown thread role sid=%s, line=%s, role=%s", sid,line_no, role)
                continue
            messages.append({"role":role,"content": row.get("content", "")})

        messages = self._trim_orphan_tool_use(messages)
        from flora_claude.core.compact.budget import truncate_tool_results
    ...
    # 将压缩后的消息写回 thread.jsonl,并且原来的文件的内容备份为 thread_<ts>.jsonl.bak
