...
from flora_claude.core.permissions.policy import (
    DEFAULT_POLICIES,
    PermissionDecision,
    ToolPolicy,
    matches_outside_cwd,
...
class PermissionManager:
    def __init__(
        ...
        *,
        ...
        # (session_id, tool_name) -> "allow" | "deny" | (session内存, 重启后丢失)
        ...
        self._persistent_always: dict[str, str] = (
            load_policy_file(self._policy_file) if policy_file is not None else {}
        )
        # 0 表示不超时
    ...
    def evaluate(self, tool_name: str, params: dict[str, Any]) -> PermissionDecision:
        from flora_claude.core.permissions.policy import evaluate
    ...
    async def check_and_wait(
        ...
        params: dict[str, Any],
        session_id: str,
        event_emitter: Callable[[dict[str, Any]], Awaitable],
    ):
    ...
    def _apply_response(self, decision: str, session_id: str, tool_name: str) -> bool:
        allow = decision in ("always_allow", "allow_once")
