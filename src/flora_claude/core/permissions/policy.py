...
def evaluate(
    ...
    # 1.判断调用bash工具时, command 访问的路径是否是禁止访问的
    if command:
    ...
    # 2.判断调用bash工具时, command 访问的路径是否是外部路径
    if command:
        if matches_outside_cwd(command):
            return PermissionDecision.ASK

    # 3.判断调用bash工具时, command 访问的路径是否是 Policy 中直接允许访问的路径
    if command:
        for pat in policy.allow_patterns:
            if re.search(pat, command):
                return PermissionDecision.ALLOW

    # 4. 如果都不是则返回 Policy 默认的权限
