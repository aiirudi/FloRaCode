...
_DEFAULT_LOG_FILE = "~/.flora/logs/core.log"
...
def get_config() -> FloRaConfig:
    # 1. 加载默认配置
    config = FloRaConfig()  

    load_dotenv(".env", override=False)
    explicit = os.environ.get("FLORA_CONFIG")
    if explicit:
        config_paths = [Path(explicit).expanduser()]
    else:
        config_paths = [
            Path(_DEFAULT_CONFIG_PATH).expanduser(),
            Path(".flora/config.toml"),
        ]
    ...
    if "mcp" in data:
        ...
        for i, srv in enumerate(servers_raw):
            ...
            if transport not in ("stdio", "tcp"):
                raise SystemExit(f"Config error: mcp.servers[{i}].tranport must be 'stdio' or 'tcp'")
            ...
            if "args" in srv:
                ...
                if not isinstance(val, list):
                    raise SystemExit(f"Config error: mcp.servers[{i}].args must be an arry")
...
def _apply_env(config: FloRaConfig) -> None:
    ...
    port_str = os.environ.get("FLORA_PORT")
    if port_str is not None:
        try:
            config.port = int(port_str)
        except ValueError:
            raise SystemExit(f"Config error: FLORA_PORT must be an integer, got: {port_str!r}")
        
    log_level = os.environ.get("FLORA_LOG_LEVEL")
    if log_level is not None:
        config.logging.level = log_level

    log_file = os.environ.get("FLORA_LOG_FILE")
    if log_file is not None:
        config.logging.file = log_file

    log_format = os.environ.get("FLORA_LOG_FORMAT")
    if log_format is not None:
        config.logging.format = log_format
    
    max_steps_str = os.environ.get("FLORA_MAX_STEPS")
    if max_steps_str is not None:
        try:
            val = int(max_steps_str)
            if val <= 0:
                raise SystemExit(
                    "Config error: FLORA_MAX_STEPS must be a positive integer,"
