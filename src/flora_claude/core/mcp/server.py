...
from flora_claude.core.config import McpServerConfig
from flora_claude.core.mcp.client import McpClient
...
class McpServerManager:
    ...
    # 将所有已经发现的 MCP 工具注册到指定的 registry
    def register_tools(self, registry: ToolRegistry) -> None:
        for tool in self._tools:
            registry.register(tool)

    # 返回已经发现的 MCP 工具列表（用于 runner 每次 run 时注入新 registry）
    def get_tools(self) -> list[McpTool]:
        return list(self._tools)

    # 关闭所哟 MCP 连接并终止 stdio 子进程
