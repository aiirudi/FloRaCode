...
class McpClient:
    ...
    async def call_tool(self, name: str, arguments: dict[str, Any]) -> str:
        reponse = await self._call("tools/call", {"name": name, "arguments": arguments})

        parts: list[str] = []
        for item in reponse.get("content", []):
            if item.get("type") == "text":
                parts.append(str(item["text"]))
        return "\n".join(parts)


    # 后台任务：持续读取 stderr 并记录，方法 McpServer 端管道缓冲区满，从而出现一种
    ...
    # 发送 JSON-RPC 请求并等待响应；id比较用字符串兼容服务端返回字符串id
