...
from flora_claude.core.bus.envelope import JsonRpcError, JsonRpcSuccess
from flora_claude.core.bus.commands import PongResult


# 同步入口： 运行 ping 协程，连接失败时打印错误并退出
def cmd_ping(config: FloRaConfig) -> None:
    try:
...
async def _ping(config: FloRaConfig) -> None:
    t0 = time.monotonic()
    ...
    req = { 
        ...
        "id": "cli-1",
        "method": "core.ping",
        "params": {"client": f"cli/{flora_claude.__version__}"}
    }
